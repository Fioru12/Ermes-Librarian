...
def _send_syslog_audit(entry: dict[str, Any], host: str, port: int, facility_name: str) -> None:
    """Inoltra una voce di audit a un collettore Syslog in formato RFC 5424, via UDP."""
    facility_map = {
        "kern": 0,
        "user": 1,
        "mail": 2,
...
def _send_http_audit(entry: dict[str, Any], url: str, token: str) -> None:
    """Inoltra una voce di audit a un endpoint HTTPS SIEM, firmata Bearer se un token e' configurato."""
    import httpx
...
def _dispatch_remote_audit(entry: dict[str, Any]) -> None:
    """Mette in coda una voce per lo streaming SIEM, senza bloccare il chiamante.

    Il thread worker parte alla prima voce e resta in vita per il processo: lo
    streaming remoto non deve rallentare la scrittura locale, che resta la
    fonte di verita' (append_audit ha gia' scritto su disco quando questa

...
export default function OnboardingWizard({ onLibraryCreated, showNotif }: OnboardingWizardProps) {
  const [step, setStep] = useState<'welcome' | 'library' | 'connector' | 'complete'>('library')
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 'library': return (<div className='space-y-4'><h2 className='text-lg font-semibold'>Benvenuto! Crea la tua prima biblioteca</h2><p className='text-sm leading-6 text-slate-400'>Una biblioteca è una raccolta di documenti su un argomento, per esempio le procedure di un ufficio. L'assistente risponde usando solo i documenti che ci metti dentro.</p><Input label='Nome' placeholder='es. Ufficio acquisti' value={libraryName} onChange={e => setLibraryName(e.target.value)} /><Input label='Descrizione (facoltativa)' placeholder='es. Procedure, moduli e regole per gli ordini' value={libraryDescription} onChange={e => setLibraryDescription(e.target.value)} /><Button onClick={handleCreateLibrary} disabled={isCreating}>{isCreating ? 'Creazione...' : 'Crea'}</Button></div>)

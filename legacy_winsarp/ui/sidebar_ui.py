...
def _fetch_available_models() -> list[str]:
    try:
        from legacy_winsarp.core.rag_engine import fetch_ollama_models
...
def toggle_memory():
    st.session_state.memoria_attiva = not st.session_state.get("memoria_attiva", True)


def render_theme_toggle(dark_mode: bool) -> None:
    btn_label = "🌙 Tema scuro" if dark_mode else "☀️ Tema chiaro"
    st.button(btn_label, use_container_width=True, on_click=toggle_theme)


def render_memory_toggle() -> None:
    active = st.session_state.get("memoria_attiva", True)
    btn_label = "🧠 Memoria: ON" if active else "🧠 Memoria: OFF"
    st.button(btn_label, use_container_width=True, on_click=toggle_memory)
...
def execute_db_operation(action_label: str, audit_action: str, modulo_scelto: str,
    ...
    from legacy_winsarp.core.rag_engine import _chroma_lock
...
def render_sidebar(base_docs_dir: str, base_chroma_path: str, hash_file: str) -> tuple[str | None, dict, list[str]]:
    ...
    from config import cfg
    from core.governance import append_audit, validate_admin_user
    from legacy_winsarp.core.memory import clear_module_memory
    from legacy_winsarp.core.rag_engine import get_module_config
    ...
    from ui.welcome_ui import MODE_INFO, render_help_panel
    ...
    # Sidebar UI elements
    render_logo()
    render_theme_toggle(st.session_state.dark_mode)
    ...
    if modulo_scelto:
        ...
        st.button("💬 Nuova conversazione", use_container_width=True, on_click=_new_conversation)
    ...
    with st.expander("🔧 Manutenzione"):
        if st.button("🧹 Rimuovi UUID orfani", use_container_width=True):
            if not validate_admin_user(st.session_state.admin_user):
                st.error("Operazione riservata ad admin.")
                st.stop()
            from core.utils import cleanup_orphan_collections

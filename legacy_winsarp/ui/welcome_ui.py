...
def render_feedback_buttons(resp_num: int):
    ...
    def _toggle_feedback(val):
        if st.session_state[fb_key] == val:
            st.session_state[fb_key] = None
        else:
            st.session_state[fb_key] = val

    cols = st.columns([1, 1, 4])
    with cols[0]:
        st.button("Utile", key=f"fb_up_{resp_num}", use_container_width=True,
                  type="secondary" if current != "up" else "primary",
                  on_click=_toggle_feedback, args=("up",))
    with cols[1]:
        st.button("Non utile", key=f"fb_down_{resp_num}", use_container_width=True,
                  type="secondary" if current != "down" else "primary",
                  on_click=_toggle_feedback, args=("down",))
    with cols[2]:
        if current == "up":
            st.markdown(
...
def render_help_panel():
    ...
    with st.expander("Aiuto e FAQ", expanded=False):
        ...
        **I dati sono sicuri?**
        ...
        st.button("Ricarica sistema", use_container_width=True,
                  help="Forza il ricaricamento dell'app",
                  on_click=_clear_cache_and_reload)
...
def render_conversation_actions():
    ...
    with col1:
        ...
        st.button("Nuova chat", use_container_width=True, on_click=_new_chat_callback)
    with col2:

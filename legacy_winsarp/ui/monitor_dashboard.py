...
def render_dashboard_in_sidebar(cfg, session_state, modulo_scelto: str = None):
    ...
    with st.expander("📊 Dashboard KPI", expanded=False):
        ...
        try:
            ...
            cs = cache.stats()
            if cs["entries"] > 0 or cs["hits"] > 0:
                hit_pct = cs["hit_rate"] * 100
                st.caption(f"💾 Cache: {cs['entries']} entry, {hit_pct:.0f}% hit rate ({cs['hits']}H/{cs['misses']}M)")
        except Exception:
            pass
        ...
        if trends and any(t["queries"] > 0 for t in trends):
            with st.container():

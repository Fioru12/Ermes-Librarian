...
def stream_response(
    ...
    if modalita == "generazione":
        from legacy_winsarp.core.business_assistant import BusinessAssistant
    ...
    if st.session_state.get("memoria_attiva", True):
        # 1. Fast path: in-memory LRU cache (exact match, istantaneo)
        from core.response_cache import get_response_cache
        response_cache = get_response_cache()
        cached = response_cache.get(prompt, model_id, modulo_scelto)
        if cached:
            st.session_state.response_count += 1
            resp_num = st.session_state.response_count
            from legacy_winsarp.modules.winsarp import parse_response
            ...
            st.session_state.messages.append({
                ...
                "model": cached.model, "mode": modalita, "cached": True,
            })
            from core.utils import append_to_log
        ...
        # 2. Semantic path: memory system (ChromaDB + LLM match)
        from legacy_winsarp.core.memory import recall as mem_recall
    ...
    from legacy_winsarp.core.rag_engine import get_source_nodes, is_low_confidence
    from legacy_winsarp.modules.winsarp import FALLBACK_PHRASES, is_fallback
    ...
    if st.session_state.get("memoria_attiva", True) and not fallback:
        from legacy_winsarp.core.memory import remember as mem_remember
    ...
    if has_formula:
        if parsed["has_split"] and parsed["code"]:
            if modalita == "generazione":
                ...
                with col2:
                    st.button("🔄 Rigenera", key=f"regen_{resp_num}", use_container_width=True, on_click=_regen_callback)

...
from legacy_winsarp.core.winsarp.parser_rules import (
    CALL_R, CALL_P, RESET_FIELDS, FIELD_REFS, RETURN_CODES, OPERATORS,
    K_FIELD as K_FIELDS, BRACED_FIELD as BRACED, BRACKET_FIELD as BRACKET_REF,
    KEY_SUM, FIELD_CMP,
)
...
def parse_catalog() -> list[dict[str, Any]]:
    ...
    return load_catalog(CATALOGO_PATH)
...
def extract_fields(code: str) -> dict[str, Any]:
    ...
    for match in comparisons:
        # Gestiamo tuple di lunghezza diversa (se il regex cattura in modo inaspettato)
        if len(match) != 3:
            continue
        f, op, val = match
        f = int(f)
        if f not in field_cmp:
            field_cmp[f] = []
        entry = {"op": op, "val": val.strip("'").strip('"')}
...
class KnowledgeGraph:
    ...
    def validate_chain(self, start_id: int, max_depth: int = 10) -> dict:
        ...
        def _walk(fid: int, depth: int):
            ...
            compared = [int(k) for k in n.get("comparisons", {})]

...
class WorkbookRetriever:
    ...
    def find_by_codice(self, codice: int) -> FormulaEntry | None:
    ...
    def suggest_placement(self, codice: int, tipo: str, categoria: str = "", user_request: str = None, generated_formula: str = None) -> dict[str, str]:
        """Suggerisce dove inserire una formula basandosi su tipo, categoria e flusso dati."""
        entry = self.find_by_codice(codice) if isinstance(codice, int) else None
        cat = categoria or (entry.categoria if entry else "")

        # --- Iniezione suggerimenti dal Grafo ---
        graph_suggestions = []
        try:
            from legacy_winsarp.core.winsarp.formula_graph import FormulaDependencyGraph, _extract_read_write
            g = FormulaDependencyGraph()
            g.load()

            reads, writes = set(), set()
            if generated_formula:
                writes, reads = _extract_read_write(generated_formula)
            elif user_request:
                fields = set(int(f) for f in re.findall(r'\b\d{2,4}\b', user_request))
                reads, writes = fields, fields

            if reads or writes:
                graph_suggestions = g.suggest_insertion_points(reads=reads, writes=writes, max_suggestions=2)
        except Exception as e:
            _logger.warning("Errore caricamento grafo in suggest_placement: %s", e)
        # ----------------------------------------

        def _format_result(base_suggestion: dict[str, str]) -> dict[str, str]:
            if not graph_suggestions:
                return base_suggestion

            aggancio_graph = []
            ponte_graph = []
            for s in graph_suggestions:
                aggancio_graph.append(f"#{s['id']} ({s['name']}) [score {s['score']}]")
                ponte_graph.append(f"Condivide flusso dati con #{s['id']} ({s['reason']})")

            return {
                "aggancio": f"Basato su flusso dati: {', '.join(aggancio_graph)}. In alternativa: {base_suggestion['aggancio']}",
                "ponte": f"Suggerimento tecnico: {'; '.join(ponte_graph)}. Collegamento strutturale: {base_suggestion['ponte']}"
            }

        # Logica base (esistente)
        if "Subroutine" in tipo:
        ...
        if "Inizio" in tipo:
            if "Dirigenti" in cat or "Quadri" in cat:
                return _format_result({"aggancio": "Dopo #1010", "ponte": "Aggiungi P [CodiceNuovaFormula] in #1010"})
            if "Personalizzato" in cat or "Arrotondamento" in cat:
                return _format_result({"aggancio": "Tra #5 e #1000", "ponte": "Aggiungi P [CodiceNuovaFormula] in #5 o #1000"})
            return _format_result({"aggancio": "Tra #10 e #1000", "ponte": "Aggiungi P [CodiceNuovaFormula] in #10"})

        if "Fine" in tipo:
            if "Gestione" in cat:
                return _format_result({"aggancio": "Template #3000/#3001", "ponte": "Aggiungi P [CodiceNuovaFormula] in #3000"})
            if "Dirigenti" in cat or "Quadri" in cat:
                return _format_result({"aggancio": "Template #1100", "ponte": "Aggiungi P [CodiceNuovaFormula] in #1100"})
            if "A Chiamata" in cat:
                return _format_result({"aggancio": "Template #2000", "ponte": "Aggiungi P [CodiceNuovaFormula] in #2000"})
            if "Straordinario" in cat or "Turnisti" in cat:
                return _format_result({"aggancio": "Flusso #120 -> #130/#140 -> #200", "ponte": "Aggiungi P [CodiceNuovaFormula] in #130 o #140"})
            return _format_result({"aggancio": "Flusso #100 -> #110 -> #120 -> ... -> #200", "ponte": "Aggiungi P [CodiceNuovaFormula] tra #120 e #200"})

        return _format_result({"aggancio": "Da definire", "ponte": "Da definire"})
    ...
    def build_enriched_prompt(self, user_request: str) -> str:
        ...
        prompt = (
            ...
            "--- RIFERIMENTO RAPIDO SINTASSI WinSarp ---\n"
            "La sintassi compatta WinSarp ha queste regole:\n"
            "- Condizione IF:  {campo} {operatore} {valore}\n"
            "    Operatori: U (=), >, <, >U (>=), <U (<=), # (!=), ! (NOT)\n"
            "- Operatori logici: E (AND), O (OR)\n"
            "- Valori speciali: I (VERO/flag attivo), Z (FALSO/zero) — MAI quotare\n"
            "- Assegnamento: ( campo = valore )\n"
            "- Reset: ( !campo )\n"
            "- Accumulo K: ( K{campo}{A/S}{valore} )  (A=aggiungi, S=sottrai)\n"
            "- CAMPO70: ( 70='codice' ) usa campi 71-78 come I/O\n"
            "- Label Vxx: V02, V04, V10... per salti condizionali\n"
            "- Fine formula: VF\n"
            "- Chiamata catena: R N\n"
            "- Chiamata subroutine: P N\n"
            "- Pointer: {campo} dereferenza, [campo inizio catena, ]campo fine catena\n"
            "- Orari: formato hh.mm tra apici (es. '08.00', '17.30')\n"
            "- Stringhe: doppi apici (es. \"MATT\", \"POME\", \"NOTT\")\n\n"

            "--- MAPPA STRUTTURALE DEI CAMPI ---\n"
            "1-6: Totali giornalieri (1=prev, 2=eff, 3=ordinarie, 4=straord, 5=assenze)\n"
            "50-58: Flag giorno e turno (50=giorno sett, 55=festivo, 58=tipo turno)\n"
            "70-79: Campo70 operativo (70=opcode, 71-78=I/O)\n"
            "80-89: Entrate/uscite giornata (prev/eff/calc)\n"
            "100-160: Orario previsionale (intervalli 1-10)\n"
            "200-240: Timbrature effettive (201=entrata1, 221=uscita1, ...)\n"
            "250-290: Timbrature calcolate (251=entrata1, 271=uscita1, ...)\n"
            "300-399: Data e flag di contesto (300=data, 301=oggi, 302=ieri)\n"
            "400-450: Causali manuali (10 slot)\n"
            "500-570: Causali automatiche (501-510=sigle, 561-570=ore per tipo)\n"
            "600-799: Totali progressivi K (601=ore lav, 602=ordinarie, 604=straord, 608=assenze retr, 609=assenze non retr)\n"
            "800-899: Campi appoggio liberi (temp, puntatori, calcoli custom)\n"
            "900: Flag anti-loop / indicatore turno\n"
            "1000: Codice azienda | 1100: Codice dipendente | 1114: Ore settimanali\n\n"

            "CAMPI DI APPOGGIO COMUNI:\n"
            "  800 = temp straordinario / accumulo arrotondamento\n"
            "  801 = temp straordinario / puntatori\n"
            "  802 = durata intervallo / puntatori\n"
            "  803 = durata / puntatori loop\n"
            "  804 = temp / puntatori loop\n"
            "  811 = entrata intervallo per subroutine\n"
            "  812 = uscita intervallo per subroutine\n"
            "  887 = soglia straordinario settimanale\n"
            "  889 = soglia supplementare part-time\n"
            "  890 = maggiorazione diurna\n\n"

...
class AgentRunner:
    ...
    def _call_llm(self, prompt: str, model_id: str, max_tokens: int = 4096) -> str:
        """Chiamata LLM tramite helper centralizzato (OpenRouter o Ollama)."""
        from core.ai.utils import call_llm
        try:
            return call_llm(prompt=prompt, model_id=model_id, temp=0.0)
    ...
    def _guess_tool(self, query: str) -> tuple[str, str] | None:
        ...
        if ("confront" in q or "differenz" in q) and len(ids) >= 2:
            return ("compare_formulas", f"{ids[0]},{ids[1]}")
        if ("catena" in q or "valid" in q or "catena" in q) and ids:

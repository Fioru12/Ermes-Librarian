...
from legacy_winsarp.modules.winsarp import auto_fix_formula, parse_response
...
def test_catalogo():
    ...
    # Assert: almeno il 90% delle formule deve superare la validazione
    assert tasso >= soglia, (
        f"Tasso validazione troppo basso: {tasso:.1f}% (soglia: {soglia}%). "
        f"{falliscono} formule su {totale} falliscono la validazione."
    )

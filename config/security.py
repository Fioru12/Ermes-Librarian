...
class SecurityConfig:
    ...
    ADMIN_PASSWORD: str = field(
        default_factory=lambda: os.environ.get("ERMES_ADMIN_PASSWORD", "")
    )
    ADMIN_USERNAME: str = field(
        default_factory=lambda: os.environ.get("ERMES_ADMIN_USERNAME", "admin")
    )
    API_KEY: str = field(
        default_factory=lambda: os.environ.get("ERMES_API_KEY", "")
    )
    ...
    METRICS_TOKEN: str = field(
        default_factory=lambda: os.environ.get("ERMES_METRICS_TOKEN", "")
    )
    ADMIN_MAX_UPLOAD_MB: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_ADMIN_MAX_UPLOAD_MB", "50"))
    )
    PROVIDER_ALLOWED_HOSTS: tuple[str, ...] = field(
        default_factory=lambda: tuple(host.strip().lower() for host in os.environ.get(
    ...
    SESSION_TTL_HOURS: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_SESSION_TTL_HOURS", "24"))
    )

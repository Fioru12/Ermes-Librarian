...
class RAGConfig:
    ...
    TOP_K_INITIAL: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_TOP_K_INITIAL", "10"))
    )
    TOP_K_FINAL: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_TOP_K_FINAL", "3"))
    )
    ...
    EMBEDDING_DIMENSION: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_EMBEDDING_DIMENSION", "768"))
    )
    EMBEDDING_BATCH_SIZE: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_EMBEDDING_BATCH_SIZE", "32"))
    )
    ...
    CHUNK_SIZE: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_CHUNK_SIZE", "1000"))
    )
    CHUNK_OVERLAP: int = field(
        default_factory=lambda: int(os.environ.get("ERMES_CHUNK_OVERLAP", "200"))
    )
    ...
    RERANKER_NEURAL_ENABLED: bool = field(
        default_factory=lambda: os.environ.get("ERMES_RERANKER_NEURAL", "1").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    HYDE_ENABLED: bool = field(
        default_factory=lambda: os.environ.get("ERMES_HYDE_ENABLED", "0").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    SEARCH_CACHE_ENABLED: bool = field(
        default_factory=lambda: os.environ.get("ERMES_SEARCH_CACHE_ENABLED", "1").strip().lower()
        in {"1", "true", "yes", "on"}
    )

...
def test_evidence_only_never_calls_a_provider(monkeypatch):
    monkeypatch.setattr(
        evidence_assistant, "cfg", evidence_assistant.cfg.replace(LIBRARY_ASSISTANT_MODE="evidence_only")
    )
    ...
    answer, coverage, reason = evidence_assistant.answer_from_evidence(
        "Quando ferie?", _citations(), mode="evidence_only"
    )
...
def test_local_answer_keeps_evidence_markers(monkeypatch):
    monkeypatch.setattr(
        evidence_assistant, "cfg", evidence_assistant.cfg.replace(LIBRARY_ASSISTANT_MODE="local_ollama")
    )
    monkeypatch.setattr(evidence_assistant, "_call_ollama", lambda _: "Richiedile cinque giorni prima.[1]")

    answer, coverage, reason = evidence_assistant.answer_from_evidence(
        "Quando ferie?", _citations(), mode="local_ollama"
    )
...
def test_approved_provider_is_explicit_and_keeps_evidence_markers(monkeypatch):
    ...
    monkeypatch.setattr(
        evidence_assistant,
        "_call_approved_provider",
        lambda _, name: calls.append(name) or "Richiedile cinque giorni prima.[1]",
    )
    ...
    answer, coverage, reason = evidence_assistant.answer_from_evidence(
        ...
        provider_name="OpenRouter aziendale",
    )

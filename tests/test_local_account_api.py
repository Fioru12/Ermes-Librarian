...
def test_creating_local_account_rejects_weak_or_duplicate_credentials(tmp_path, monkeypatch):
    ...
    with pytest.raises(HTTPException, match="maiuscola"):
        asyncio.run(
            create_local_account(
                CreateLocalAccountRequest(username="maria", role="viewer", password="weakpass1!"), actor
            )
        )

    asyncio.run(
        create_local_account(
            CreateLocalAccountRequest(username="maria", role="viewer", password="StrongUser!123"), actor
        )
    )
    with pytest.raises(HTTPException, match="Esiste gia"):
        asyncio.run(
            create_local_account(
                CreateLocalAccountRequest(username="maria", role="viewer", password="AnotherStrong!123"), actor
            )
        )
...
def test_updating_local_account_never_audits_password_and_keeps_an_active_admin(tmp_path, monkeypatch):
    ...
    asyncio.run(
        create_local_account(
            CreateLocalAccountRequest(username="anna", role="admin", password="StrongAdmin!456"), actor
        )
    )
    ...
    with pytest.raises(HTTPException, match="tuo stesso"):
        asyncio.run(
            update_local_account("anna", UpdateLocalAccountRequest(active=False), {"username": "anna", "role": "admin"})
        )
